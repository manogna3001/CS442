...
class ScorecardModel with ChangeNotifier {
  ...
  void updatescore(ScoreCategory category, DiceModel dice, BuildContext context) {
    if (dice.values.isNotEmpty) {
      _scorecard.registerScore(category, dice.values.cast<int>());

      dice.clear();

      dice.canHoldDice = false;

      notifyListeners(); 

      if (_scorecard.completed) {
        GameoverAlert().gameover(context, _scorecard).then((_) {
          _scorecard.clear();
          dice.clear(); // Clear everything when the game is over
